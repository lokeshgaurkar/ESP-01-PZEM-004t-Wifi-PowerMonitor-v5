...
const char* default_ap_ssid = "PowerMonitor";
const char* default_ap_password = "11223344";
char ap_ssid[33] = {0};
char ap_password[65] = {0};
const char* hostname = "powermonitor";

// ===== HTTP + Captive Portal Feature =====
ESP8266WebServer server(80);
DNSServer dnsServer;

// ===== PZEM Energy Meter Feature =====
...
#define WIFI_CFG_ADDR 160
#define WIFI_CFG_MAGIC 0x504D5746UL

// ===== Billing Calculator Feature =====
struct Tariff {
  float meter;
  float ship;
  float tax;
...
// ===== Wi-Fi Credentials Storage Feature =====

bool isPrintableAscii(const char* s) {
  if (!s) return false;
  while (*s) {
...
void loadWiFiConfig() {
  ...
  bool valid = (wifiCfg.magic == WIFI_CFG_MAGIC) &&
               (strlen(wifiCfg.ssid) >= 1) &&
               (strlen(wifiCfg.ssid) <= 32) &&
               (strlen(wifiCfg.password) >= 8) &&
               (strlen(wifiCfg.password) <= 64) &&
               isPrintableAscii(wifiCfg.ssid) &&
               isPrintableAscii(wifiCfg.password);

  if (!valid) {
    strncpy(ap_ssid, default_ap_ssid, sizeof(ap_ssid) - 1);
    strncpy(ap_password, default_ap_password, sizeof(ap_password) - 1);
    return;
  }

  strncpy(ap_ssid, wifiCfg.ssid, sizeof(ap_ssid) - 1);
  strncpy(ap_password, wifiCfg.password, sizeof(ap_password) - 1);
}

void saveWiFiConfig(const String& ssid, const String& pass) {
  memset(&wifiCfg, 0, sizeof(wifiCfg));
  wifiCfg.magic = WIFI_CFG_MAGIC;
  ssid.toCharArray(wifiCfg.ssid, sizeof(wifiCfg.ssid));
  pass.toCharArray(wifiCfg.password, sizeof(wifiCfg.password));
  EEPROM.put(WIFI_CFG_ADDR, wifiCfg);
  EEPROM.commit();
}
// ===== Runtime Performance Monitoring Feature =====
...
// ===== Live Electrical Measurements Feature =====
...
background:linear-gradient(145deg,#1b264c,#0f1735);
color:#d9e5ff;
font-size:12px;
letter-spacing:.04em;
text-transform:uppercase;
...
0%{transform:translateY(0) scale(1);text-shadow:0 0 0 rgba(255,231,109,0)}
35%{transform:translateY(-1px) scale(1.04);text-shadow:0 0 8px rgba(255,231,109,.45)}
100%{transform:translateY(0) scale(1);text-shadow:0 0 0 rgba(255,231,109,0)}
}
35%{transform:translateY(-1px) scale(1.08);text-shadow:0 0 16px rgba(255,231,109,.9)}
100%{transform:translateY(0) scale(1);text-shadow:0 0 0 rgba(255,231,109,0)}
}
...
<button id="wifiSettingsBtn" class="wifiChip" title="Wi-Fi Settings">Wi-Fi</button>
...
const nextSsid=prompt("Enter new Wi-Fi SSID","")
if(nextSsid===null) return
const nextPass=prompt("Enter new Wi-Fi password (min 8 chars)","")
if(nextPass===null) return
if(nextSsid.trim().length<1){alert("SSID required");return}
if(nextSsid.trim().length>32){alert("SSID max 32 chars");return}
if(nextPass.length<8){alert("Password min 8 chars");return}
if(nextPass.length>64){alert("Password max 64 chars");return}
fetch(`/wifi?ssid=${encodeURIComponent(nextSsid.trim())}&pass=${encodeURIComponent(nextPass)}`)
.then(r=>r.text())
.then(msg=>alert(msg))
.catch(()=>alert("Failed to save Wi-Fi settings"))
}

function handlePowerSpike(nextPower){
const pEl=document.getElementById("p")
if(!pEl) return
const now=Date.now()
const jump=nextPower-lastPowerForSpike
const crossedGate=nextPower>=25
if(lastPowerForSpike>0 && crossedGate && jump>=20 && (now-lastSpikeAt)>1200){
pEl.classList.remove("spike")
void pEl.offsetWidth
pEl.classList.add("spike")
lastSpikeAt=now
}
lastPowerForSpike=nextPower
}function update(){
...
handlePowerSpike(Number(data.p)||0)
setMetric("e",data.e)
setMetric("f",data.f)
...
document.getElementById("wifiSettingsBtn").addEventListener("click",()=>{vibrate();window.location.href="/wifi";});
...
// ===== OTA Firmware Update Web UI Feature =====
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>PowerMonitor Wi-Fi Settings</title>
<link rel="icon" type="image/png" href="/favicon.png">
<style>
:root{
--bg:#0b1020;
...
.liveDot{width:10px;height:10px;border-radius:50%;background:#27d3a5;box-shadow:0 0 0 0 rgba(39,211,165,.65);animation:pulse 1.8s infinite;}
...
.inputgroup{display:flex;flex-direction:column;gap:6px;margin-bottom:12px;}
.inputgroup label{font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.08em;}
.inputgroup input{
padding:11px;
border-radius:12px;
border:1px solid var(--line);
...
.inputgroup input:focus{border-color:#65b2ff;box-shadow:0 0 0 3px rgba(101,178,255,.2);}
...
button:active,a.btn:active{transform:translateY(1px) scale(.995)}

.saveBtn{background:linear-gradient(145deg,#27d3a5,#1fb889);color:#05261e;box-shadow:0 10px 24px rgba(39,211,165,.25);}
.backBtn{background:linear-gradient(145deg,#6ca1ff,#4977ff);color:#eaf2ff;box-shadow:0 10px 24px rgba(93,141,255,.24);}
...
padding:10px 12px;
border-radius:12px;
border:1px solid rgba(149,170,220,.3);
...
.status.ok{border-color:rgba(39,211,165,.45);color:#b5ffd9;}
.status.err{border-color:rgba(255,95,112,.45);color:#ffc3cb;}

@keyframes rise{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
@keyframes pulse{0%{box-shadow:0 0 0 0 rgba(39,211,165,.65)}70%{box-shadow:0 0 0 12px rgba(39,211,165,0)}100%{box-shadow:0 0 0 0 rgba(39,211,165,0)}}
</style>
</head>
<body>
<div class="shell">
  <div class="header"><span class="liveDot"></span>Wi-Fi Settings</div>
  <div class="panel">
    <p class="sub">Update Access Point name and password. Device will reboot automatically after save.</p>
    <form id="wifiForm">
      <div class="inputgroup">
        <label for="ssid">Wi-Fi Name (SSID)</label>
        <input id="ssid" type="text" maxlength="32" required placeholder="PowerMonitor">
      </div>
      <div class="inputgroup">
        <label for="pass">Wi-Fi Password</label>
        <input id="pass" type="password" minlength="8" maxlength="64" required placeholder="Minimum 8 characters">
      </div>
      <div class="actions">
        <button type="submit" class="saveBtn">Save & Reboot</button>
        <a class="btn backBtn" href="/">Back to Dashboard</a>
      </div>
    </form>
    <div id="status" class="status">Waiting for input...</div>
    <div class="note">Tip: reconnect using the new SSID/password after reboot.</div>
  </div>
</div>
...
if(ssid.length<1 || ssid.length>32){setStatus("Invalid SSID (1-32 chars)",false);return}
if(pass.length<8 || pass.length>64){setStatus("Invalid password (8-64 chars)",false);return}
setStatus("Saving Wi-Fi settings...",true)
fetch(`/wifi-save?ssid=${encodeURIComponent(ssid)}&pass=${encodeURIComponent(pass)}`)
.then(async r=>({ok:r.ok,text:await r.text()}))
.then(res=>{
setStatus(res.text,res.ok)
if(res.ok){
setTimeout(()=>{ setStatus("Device restarting... reconnect to new Wi-Fi in a few seconds.",true) },500)
}
})
.catch(()=>setStatus("Failed to save settings",false))
})
</script>
</body>
</html>
...
// ===== Route: OTA Update Page =====
void handleUpdatePage() {
  server.send_P(200, "text/html", updatePage);
}

// ===== Route: Wi-Fi Settings Page =====
void handleWiFiPage() {
  server.send_P(200, "text/html", wifiPage);
}
...
// ===== Route: Save Wi-Fi AP Credentials =====
void handleWiFiSettings() {
  ...
  if (ssid.length() < 1 || ssid.length() > 32) {
    server.send(400, "text/plain", "Invalid SSID (1-32 chars)");
    return;
  }
  if (pass.length() < 8 || pass.length() > 64) {
    server.send(400, "text/plain", "Invalid password (8-64 chars)");
    return;
  }

  saveWiFiConfig(ssid, pass);
  server.send(200, "text/plain", "Wi-Fi settings saved. Rebooting device...");
  delay(500);
  ESP.restart();
}
// ===== Route: JSON Live Data API =====
void handleData() {
  updateMeasurements();
...
void setup() {
  ...
  server.on("/wifi-save", handleWiFiSettings);
